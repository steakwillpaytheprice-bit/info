...
def get_info():
    try:
        ...
        if not login or 'token' not in login:
            return json.dumps({"error": "Login failed"}, indent=2), 401, {'Content-Type': 'application/json; charset=utf-8'}

        # Fetch player info
        data = get_player_personal_show(login['serverUrl'], login['token'], uid_int)
        if not data:
